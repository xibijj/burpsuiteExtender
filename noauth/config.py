...
xml_recmd = '<[\w+]{0,20}(?:{recmd})[\w+]{0,20}>.*?</[\w+]{0,20}(?:{recmd})[\w+]{0,20}>'.replace('{recmd}', p_keys)

# 更新匹配中文正则
personalinfo_recmd = '{json}|{xml}'.format(json=json_recmd, xml=xml_recmd)
...
# 自定义忽略文件类型,小写敏感
filter_files = ['.css','.js', '.jpg', '.jpeg', '.gif', '.png', '.bmp', '.swf', '.svg', '.apk', '.ipa', '.ico', '.woff', '.ttf', '.zip' ,'.so', '.jar', '.json', '.gz', '.mp4' ,'.m3u8', '.svc', '.woff', '.deb', '.bz2', '.gpg', '.lzma', '.mp3', '.xml', '.txt', '.pbf']
...
idor_rule = {
                ...
                'result': personalinfo_recmd # 敏感信息的判断标准
            }